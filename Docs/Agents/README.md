...
| 4 | [GIT_RULES.md](GIT_RULES.md) | What you may do to the repository — agents don't commit, `.meta` pairing, YAML conflicts, handoff | Any task that touches git state, and every handoff |
...
- **Agents change files, humans change history.** No `commit`, no `add`, no `push` — propose the command instead. Enforced by `.claude/settings.json`, not just documented ([GIT_RULES.md §0.1](GIT_RULES.md)).
